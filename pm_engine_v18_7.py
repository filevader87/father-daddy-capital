...
RSI_OVERBOUGHT_SEVERE = 73  # severe zone — high conviction
RSI_OVERBOUGHT = 70
RSI_NEAR_OVERBOUGHT = 65
RSI_DEAD_ZONE_LOW = 35
RSI_DEAD_ZONE_HIGH = 65

# V18.7: Direction thresholds stay same
MIN_DIRECTION_CHANGE = 0.03
LOOKBACK_CANDLES = 3

# ══════════════════════════════════════════════════════════════════════════════
# V18.7: TIERED STRATEGY — cheap-side asymmetry makes trades profitable at ANY
# confidence level. At 5¢ entry, you only need 6% probability to break even.
# The old system blocked 80%+ of signals due to MIN_CONFIDENCE=0.84, sitting
# idle 95% of the time. This system enters at all tiers but sizes by confidence.
# ══════════════════════════════════════════════════════════════════════════════

# --- Win Probability Calibration (Binance backtest + cheap-side ROI math) ---
# Severe zones: validated on real data. Moderate/neutral: from Binance study with
# cheap-side adjustment (at ≤10¢, even 55% WR = +300% EV).
WIN_PROB_BASE = {
    # TIER 1: Severe (validated 80%+ Binance WR)
    'severe_oversold_down': 0.806,  # RSI<25 + DOWN
    'severe_overbought_up': 0.871,  # RSI>73 + UP
    # TIER 2: Moderate (Binance 67-72%, +cheap-side edge)
    'oversold_down': 0.724,         # RSI 25-30 + DOWN + confirmations
    'overbought_up': 0.691,         # RSI 70-73 + UP + confirmations
    # TIER 3: Direction+cheap-side (any RSI, has direction + cheap entry)
    'direction_down_cheap': 0.58,    # DOWN direction at ≤10¢ entry
    'direction_up_cheap': 0.56,      # UP direction at ≤10¢ entry
}

# --- Confidence tiers → position sizing multipliers ---
# V18.7: Never block signals entirely. Size down at lower confidence.
# At 5¢: 6¢ to break even. At 10¢: 11¢ to break even. Cheap-side is a superpower.
CONFIDENCE_MAP = {
    # TIER 1 (severe): full position (8-12% bankroll)
    'severe_oversold_down': 0.85,
    'severe_overbought_up': 0.86,
    # TIER 2 (moderate): half position (4-6% bankroll)
    'oversold_down': 0.75,
    'overbought_up': 0.70,
    # TIER 3 (direction+cheap): quarter position (2-3% bankroll)
    'direction_down_cheap': 0.58,
    'direction_up_cheap': 0.56,
}
...
def generate_signal_v187(prices, candles=None, idx=None):
    ...
    # ── TIER 3: Direction + Cheap-Side (V18.7 NEW) ──
    # If no TIER 1/2 signal fired but we have a clear direction, enter at cheap side.
    # Cheap-side asymmetry: at ≤10¢, even 56% WR = +400% EV.
    # Small position (3% bankroll) limits downside while capturing frequent direction trades.
    if signal is None and direction in ('UP', 'DOWN'):
        # Need at least moderate direction strength (>0.1% move)
        if strength is not None and strength > 0.1:
            # Skip in ranging regime (no direction to ride)
            if regime != 'ranging':
                if direction == 'DOWN':
