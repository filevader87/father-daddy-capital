#!/usr/bin/env python3
"""V19.7m Paper Trading Engine — Metric semantics fix + PBot research profiles.
...
def fetch_clob_book(token_id):
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "FDC-Paper/19.7m"})
...
def init_profile_state(profile_key):
    ...
    return {
        ...
        "market_opportunities": 0,        # valid compatible market exists
        "signal_opportunities": 0,        # valid strategy signal exists
        "signal_market_opportunities": 0, # signal + market overlap
        "executable_opportunities": 0,    # signal + market + price + EV + book gates
        "paper_trades_opened": 0,
        "paper_trades_resolved": 0,
        "valid_opportunities": 0,         # LEGACY alias for executable_opportunities
        "false_accepts": 0,
...
def run_paper_cycle():
    ...
    print(f"  CORE_UP paper_trades:               {paper_trades_core}")
