...
def run_simulation():
    ...
    for fidx, fpath in enumerate(valid_files):
        ...
        for cid, aid_list in pairs:
            ...
            for i in range(24, n_pts, step):
                ...
                # V21.7.4 §4: 7-bucket classification
                BUCKETS_V2174 = {
                    "SUB_FLOOR": (0.00, 0.03), "PRIMARY_LOW": (0.03, 0.05),
                    "PRIMARY_PREFERRED": (0.05, 0.08), "PRIMARY_HIGH": (0.08, 0.12),
                    "ADJACENT_HIGH": (0.12, 0.20), "MIDRANGE_BLOCKED": (0.20, 0.40),
                    "CONVEXITY_GONE": (0.40, 1.00),
                }
                bucket = "CONVEXITY_GONE"
                for bn, (blo, bhi) in BUCKETS_V2174.items():
                    if blo <= eff_price < bhi:
                        bucket = bn; break
    ...
    with open(out / "v2174_adj_bucket_v21_baseline.json", 'w') as f:
        json.dump(results, f, indent=2, default=str)
    
    # ═══════════════════════════════════════════════════════════════════
    # V21.7.4 §4: ADJACENT BUCKET EV DIAGNOSTIC OUTPUTS
    # ═══════════════════════════════════════════════════════════════════
    out_dir = Path("/home/naq1987s/father-daddy-capital/output/v2174")
    out_dir.mkdir(parents=True, exist_ok=True)
    
    BUCKETS_ORDERED = ["SUB_FLOOR","PRIMARY_LOW","PRIMARY_PREFERRED","PRIMARY_HIGH",
                       "ADJACENT_HIGH","MIDRANGE_BLOCKED","CONVEXITY_GONE"]
    
    bucket_reports = {}
    for bname in BUCKETS_ORDERED:
        bt = [t for t in trades if t.get('bucket') == bname]
        if not bt:
            bucket_reports[bname] = {"observations": 0, "classification": "NO_DATA"}
            continue
        bw = [t for t in bt if t['won']]
        bl = [t for t in bt if not t['won']]
        gpnl = sum(t['pnl'] for t in bt)
        gw = sum(t['pnl'] for t in bw) if bw else 0
        gl = abs(sum(t['pnl'] for t in bl)) if bl else 0.001
        pf_b = gw / max(gl, 0.001)
        max_ls = cur = 0
        for t in bt:
            if not t['won']: cur += 1; max_ls = max(max_ls, cur)
            else: cur = 0
        rsi_vals = [t['rsi'] for t in bt]
        vel_vals = [abs(t['velocity']) for t in bt if t.get('velocity')]
        time_vals = [t['time_pct'] for t in bt]
        
        # Slippage-adjusted PnL (slip = entry * 0.8% on top of base)
        slip_adj_pnl = sum(t['pnl'] - t['entry'] * 0.008 * t['shares'] for t in bt)
        
        bucket_reports[bname] = {
            "observations": len(bt),
            "shadow_entries": len(bt),
            "resolved_entries": len(bt),
            "wins": len(bw), "losses": len(bl),
            "wr": round(len(bw)/len(bt)*100, 2),
            "gross_pnl": round(gpnl, 4),
            "slippage_adj_pnl": round(slip_adj_pnl, 4),
            "ev_per_trade": round(slip_adj_pnl/len(bt), 4) if bt else 0,
            "pf": round(pf_b, 3),
            "max_loss_streak": max_ls,
            "mean_tte": round(float(np.mean([(1-t['time_pct'])*300 for t in bt])), 1) if bt else 0,
            "mean_v15": round(float(np.mean(vel_vals[-15:])) if vel_vals else 0, 6),
            "mean_v30": round(float(np.mean(vel_vals[-30:])) if len(vel_vals)>=30 else float(np.mean(vel_vals)) if vel_vals else 0, 6),
            "mean_v60": round(float(np.mean(vel_vals[-60:])) if len(vel_vals)>=60 else float(np.mean(vel_vals)) if vel_vals else 0, 6),
            "regime_distribution": {r: sum(1 for t in bt if t['regime']==r) for r in set(t['regime'] for t in bt)},
            "mean_spread": round(float(np.mean([t['entry']*0.01 for t in bt])), 6) if bt else 0,
            "mean_rsi": round(float(np.mean(rsi_vals)), 2) if rsi_vals else 0,
            "mean_cont_score": 0,
        }
    
    # ADJACENT_HIGH decision logic (§4)
    adj = bucket_reports.get("ADJACENT_HIGH", {})
    adj_res = adj.get("resolved_entries", 0)
    adj_ev = adj.get("ev_per_trade", 0)
    adj_pf = adj.get("pf", 0)
    adj_slip_ev = adj.get("slippage_adj_pnl", 0) / max(adj_res, 1)
    
    if adj_res >= 50 and adj_ev > 0 and adj_pf >= 1.25 and adj_slip_ev > 0:
        adj_class = "ADJACENT_HIGH_BUCKET_PAPER_CANDIDATE"
    elif adj_ev > 0 and adj_pf >= 1.25 and adj_res > 0:
        adj_class = "ADJACENT_HIGH_PROMISING_INSUFFICIENT_SAMPLES"
    else:
        adj_class = "LIVE_BUCKET_RESTRICTION_JUSTIFIED"
    
    # Regime signature
    win_trades = [t for t in trades if t['won']]
    lose_trades = [t for t in trades if not t['won']]
    regime_sig = {
        "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "source": "PMXT_BACKTEST",
        "total_trades": len(trades),
        "winning_velocity": {"v15_median": round(float(np.median([abs(t['velocity']) for t in win_trades[-15:]])), 6) if len(win_trades)>=15 else 0,
                              "v30_median": round(float(np.median([abs(t['velocity']) for t in win_trades[-30:]])), 6) if len(win_trades)>=30 else 0,
                              "v60_median": round(float(np.median([abs(t['velocity']) for t in win_trades[-60:]])), 6) if len(win_trades)>=60 else 0},
        "losing_velocity":  {"v15_median": round(float(np.median([abs(t['velocity']) for t in lose_trades[-15:]])), 6) if len(lose_trades)>=15 else 0,
                              "v30_median": round(float(np.median([abs(t['velocity']) for t in lose_trades[-30:]])), 6) if len(lose_trades)>=30 else 0,
                              "v60_median": round(float(np.median([abs(t['velocity']) for t in lose_trades[-60:]])), 6) if len(lose_trades)>=60 else 0},
        "winning_tte_median": round(float(np.median([(1-t['time_pct'])*300 for t in win_trades])), 1) if win_trades else 0,
        "losing_tte_median": round(float(np.median([(1-t['time_pct'])*300 for t in lose_trades])), 1) if lose_trades else 0,
        "winning_rsi_median": round(float(np.median([t['rsi'] for t in win_trades])), 2) if win_trades else 0,
        "losing_rsi_median": round(float(np.median([t['rsi'] for t in lose_trades])), 2) if lose_trades else 0,
        "resolved_up_down_ratio": f"{sum(1 for t in trades if t['side']=='UP')}:{sum(1 for t in trades if t['side']=='DOWN')}",
        "spread_slippage_profile": {"mean_spread": round(float(np.mean([t['entry']*0.01 for t in trades])), 6),
                                     "mean_slip": round(float(np.mean([t['entry']*0.005 for t in trades])), 6)},
        "higher_timeframe_trend": {"TRENDING_DOWN": sum(1 for t in trades if t.get('regime')=='TRENDING_DOWN'),
                                    "TRENDING_UP": sum(1 for t in trades if t.get('regime')=='TRENDING_UP'),
                                    "RANGING": sum(1 for t in trades if t.get('regime')=='RANGING')},
        "bucket_distribution": {b: sum(1 for t in trades if t.get('bucket')==b) for b in BUCKETS_ORDERED},
    }
    ...
    ev_report = {
        ...
        "adjacent_high_decision": adj_class,
        "adjacent_high_criteria": {"resolved_required": 50, "resolved_actual": adj_res,
                                    "ev_required": 0, "ev_actual": round(adj_ev, 4),
                                    "pf_required": 1.25, "pf_actual": round(adj_pf, 3),
                                    "slippage_adj_ev_required": 0, "slippage_adj_ev_actual": round(adj_slip_ev, 4)},
        "bucket_analysis": bucket_reports,
        "pmxt_vs_live_caveat": "PMXT shows historical profitability. DO NOT promote without live-shadow settlement evidence.",
        "live_bucket_rules_unchanged": True, "btc_live_bucket": "3-12c ONLY",
        "eth_sol_live": False, "up_profiles_live": False, "swarm_active": False,
    }
    
    # Write 4 required output files
    ...
    with open(out_dir / "btc_adjacent_bucket_settlements.jsonl", 'w') as f:
        for t in trades: f.write(json.dumps({k:v for k,v in t.items() if k in ('tier','side','entry','bucket','won','pnl','settlement','regime','time_pct','rsi')}, default=str) + "\n")
    with open(out_dir / "pmxt_vs_live_regime_signature_report.json", 'w') as f:
        json.dump(regime_sig, f, indent=2, default=str)
    
    # Print bucket table
    print("\n" + "=" * 95)
    print("V21.7.4 §4: BTC ADJACENT BUCKET EV DIAGNOSTIC")
    print("=" * 95)
    print(f"\n{'BUCKET':<22s} {'OBS':>6s} {'WR%':>6s} {'GrossPnL':>10s} {'SlipAdjPnL':>12s} {'EV/trade':>9s} {'PF':>6s} {'MaxLS':>5s} {'MeanTTE':>8s}")
    print("-" * 95)
    for bname in BUCKETS_ORDERED:
        br = bucket_reports.get(bname, {})
    ...
    print(f"\nADJACENT_HIGH Decision: {adj_class}")
    print(f"  Resolved: {adj_res}/50 | EV: {adj_ev:.4f}/0 | PF: {adj_pf:.3f}/1.25 | Slip-adj EV: {adj_slip_ev:.4f}/0")
    print("\n⚠ Do NOT promote 12-20c from PMXT results alone. Live-shadow settlement evidence required.")
    print("\nV2174 Output files:")
