#!/usr/bin/env python3
"""V19.7j Paper Trading Engine — Micro-live readiness + dry-run broker + kill switches.

NO REAL ORDERS unless ALL 6 gates pass AND user manually enables MICRO_LIVE mode.
...
LIVE_ELIGIBLE = {
    ...
    "paper_only_profiles": ["BIDIRECTIONAL_SHADOW", "PARABOLIC_RESEARCH"],
...
def dry_run_broker(order_details, kill_switches, daily_pnl=0, weekly_pnl=0, open_positions=0):
    ...
    if would_execute:
        result["reason_order_allowed"] = "ALL GATES PASSED — would place live order"
    else:
        parts = []
        if not live_allowed:
...
def fetch_clob_book(token_id):
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "FDC-Paper/19.7j"})
...
def run_paper_cycle():
    ...
    for profile_key, profile_cfg in PROFILES.items():
        ...
        for ak in assets_to_trade:
            ...
            for c in contracts[:3]:
                ...
                if dry_result.get("would_execute"):
    ...
    if live_allowed and kill_clear and all_passed and total_opp >= 50 and settlement_errors == 0:
        classification = "D_MICRO_LIVE_ACTIVE"
    elif MICRO_LIVE_READY and all_passed and total_opp >= 50 and settlement_errors == 0:
        classification = "C_MICRO_LIVE_READY_DRY_RUN"
    elif all_passed and total_opp >= 50 and settlement_errors == 0:
    ...
    print(f"  PAPER_GATE_PASSED: {PAPER_GATE_PASSED}")
    print(f"  SETTLEMENT_GATE_PASSED: {SETTLEMENT_GATE_PASSED}")
    print(f"  EXECUTION_GATE_PASSED: {EXECUTION_GATE_PASSED}")
    print(f"  LIVE_CONFIRMATION_FLAG: {LIVE_CONFIRMATION_FLAG}")
    print(f"  Live permission: {'✅ ALLOWED' if live_allowed else '❌ BLOCKED'} {live_reasons if not live_allowed else ''}")
    print(f"  Kill switches: {'✅ CLEAR' if kill_clear else '❌ TRIGGERED'} {kill_triggered if not kill_clear else ''}")
    print(f"  Can set MICRO_LIVE_READY: {'✅ YES' if can_set_micro_live else '❌ NOT YET'}")
    print(f"  ── Paper Stats ──")
    print(f"  Paper opportunities: {total_opp}/50")
    print(f"  Paper trades: {total_trades}")
    print(f"  Paper WR: {core['wins'] / max(total_trades, 1) * 100:.1f}%")
    print(f"  Paper net EV: ${net_pnl:.2f}")
    print(f"  Paper PF: {pf:.2f}")
    print(f"  Paper DD: {max_dd * 100:.1f}%")
    print(f"  Settlement errors: {settlement_errors}")
    print(f"  False accepts: {false_accepts}")
    print(f"  Stale books: {stale_books}")
    ...
    if can_set_micro_live and not MICRO_LIVE_READY:
        ...
        print(f"     Set MICRO_LIVE_READY=True in config to advance to C_MICRO_LIVE_READY_DRY_RUN")
    elif not can_set_micro_live:
        still_need = []
        if total_opp < 50: still_need.append(f"{50-total_opp} more opps")
    ...
    print(f"\n  Live-eligible strategy: {LIVE_ELIGIBLE['profile']} / {LIVE_ELIGIBLE['asset']} / {LIVE_ELIGIBLE['allowed_direction']} / {LIVE_ELIGIBLE['allowed_timeframes']}")
