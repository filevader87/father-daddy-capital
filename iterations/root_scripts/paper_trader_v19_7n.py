#!/usr/bin/env python3
"""V19.7n Paper Trading Engine — Metric semantics hardening + PBot benchmark harness.
...
def fetch_clob_book(token_id):
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "FDC-Paper/19.7n"})
...
def init_profile_state(profile_key):
    ...
    return {
        ...
        "blocked_candidates": 0,         # positions that had signal but failed a gate
...
def run_paper_cycle():
    ...
    print(f"  CORE_UP blocked_candidates:         {core.get('blocked_candidates', 0)}")
