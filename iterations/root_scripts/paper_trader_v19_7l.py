#!/usr/bin/env python3
"""V19.7l Paper Trading Engine — Discovery recovery patch + profile-isolated readiness + scarcity diagnostics.

NO REAL ORDERS unless ALL 6 gates pass AND user manually enables LIVE_CONFIRMATION_FLAG.
Uses C...[truncated]
...
def fetch_clob_book(token_id):
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "FDC-Paper/19.7l"})
...
def run_paper_cycle():
    ...
    core_opps = core.get("valid_opportunities", 0)
    ...
    # ━━ EVENT-BASED OPPORTUNITY MONITOR ━━
    bidir = load_profile_state("BIDIRECTIONAL_SHADOW")
    para = load_profile_state("PARABOLIC_RESEARCH")
    opps_core = core.get("valid_opportunities", 0)
    opps_bidir = bidir.get("valid_opportunities", 0)
    opps_para = para.get("valid_opportunities", 0)
    total_exec_opp = opps_core + opps_bidir + opps_para
    
    print(f"\n  ━━ OPPORTUNITY MONITOR ━━")
    print(f"  Executable opps total:        {total_exec_opp}")
    print(f"  Executable opps CORE_UP:      {opps_core}")
    print(f"  Executable opps BIDIR_SHADOW: {opps_bidir}")
    print(f"  Executable opps PARABOLIC:    {opps_para}")
    print(f"  Opps needed remaining:        {max(0, 50 - opps_core)} (CORE_UP)")
    ...
    print(f"\n  ━━ PROFILE-ISOLATED READINESS ━━")
    for pk, st, label in [("CORE_UP", core, "LIVE CANDIDATE"), ("BIDIRECTIONAL_SHADOW", bidir, "PAPER ONLY"), ("PARABOLIC_RESEARCH", para, "PAPER ONLY")]:
        opps = st.get("valid_opportunities", 0)
