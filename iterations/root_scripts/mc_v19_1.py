...
V19.1 adds: dead zone filter, correlation limit, confluence sizing,
...
def simulate_cycle(bd, prices, rng):
    ...
    # V19.1 #1: Dead zone filter
    if entry_price > DEAD_ZONE_LOW and entry_price < DEAD_ZONE_HIGH:
        return bd, None  # Rejected by dead zone
...
def main():
    ...
    print(f"  Dead Zone: {DEAD_ZONE_LOW*100:.0f}-{DEAD_ZONE_HIGH*100:.0f}¢ | Correlation: {MAX_SAME_DIRECTION} same-dir max")
