#!/usr/bin/env python3
"""
Father Daddy Capital — Polymarket Engine V18 (BTC 5-min Exit Mechanism)
=========================================================================
Overhaul based on live trading post-mortem (53 trades, -$126 root cause).

KEY CHANGES FROM V3 → V18:
  1. EXIT MECHANISM: Sell before expiry when position losing (time decay + stop-loss)
  2. GUARDS RESTORED: BEAR_SKIP=True, TREND_GUARD=True (were disabled!)
  3. HARD POSITION LIMIT: MAX_OPEN_POSITIONS=2 ENFORCED (had 13 concurrent!)
  4. BTC-ONLY SIGNALS: Weather/misc markets blocked from BTC signal logic
  5. DYNAMIC PRICE GATE: Only buy when ask ≤ (estWR - 0.03) per DUDD pattern
  6. PATTERN ORIENTATION: Preserve original pattern checks, never "fix" them
  7. KILL SWITCH: Daily loss $5, 50% DD halt (was $10/50% but never triggered)
  8. SIM-LIVE GAP: Model slippage (1 tick), rejection rate (5%), fill delay (1 bar)
...
Architecture:
  ...
  Three-stage exit: (1) stop-loss, (2) time-decay sell, (3) expiry settlement
  Traditional Kelly sizing: cold(2% fixed) → warm(floor Kelly) → live(full Kelly).

NO live trades until V18 passes 20-seed Monte Carlo (5/5 gates each).
"""
