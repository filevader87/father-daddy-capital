...
# V19.2: Multi-asset candle fetching
BINANCE_SYMBOLS = {
    "BTC": "BTCUSDT",
    "ETH": "ETHUSDT",
    "SOL": "SOLUSDT",
...
def fetch_candles_multi(asset: str, interval: str = '5m', limit: int = 100):
    """Fetch candles for any supported asset from Binance."""
    symbol = BINANCE_SYMBOLS.get(asset, "BTCUSDT")
    url = f"https://api.binance.com/api/v3/klines?symbol={symbol}&interval={interval}&limit={limit}"
    try:
        req = urllib.request.Request(url, headers={'User-Agent': 'FDC-V19.2'})
        ...
        log(f"  ⚠️ Failed to fetch {asset} candles: {e}")
        return None
...
def run_scan():
    ...
    # 2. Scan all assets (V19.2: multi-asset)
    ...
    for asset in assets_to_scan:
        ...
        log(f"  EMA21: ${ema21:,.4f} EMA50: ${ema50:,.4f} | VWAP: ${vwap:,.4f} | ATR: {atr:.4f} ({vol_regime}) | MACD_H: {macd_hist:.2f}")
        ...
        for p in state.get("positions", {}).values():
            if p.get("status") == "open":
                existing_slug = p.get("market_slug", "").replace("-15m", "").replace("-5m", "")
                existing_side = p.get("side", "").upper()
                # Check same base slug but opposite direction
                if existing_slug == market_slug_base and existing_side != best_side.upper():
                    log(f"  ⚠️ Straddle filter: already hold {existing_side} on {market_slug_base}, skipping opposite side")
                    state["last_scan"] = datetime.now(timezone.utc).isoformat()
                    save_state(state)
                    continue
